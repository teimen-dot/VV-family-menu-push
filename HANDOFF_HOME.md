...
- 当前分支：`home-work-20260806`，生产部署版本 `dc7a9ca`
- 最新部署：2026-08-07 已完成菜单生产止血热修；menu.ourmenu.site / admin.ourmenu.site 服务正常
- 当前数据库状态：正式 SQLite 为唯一正式可写数据源；如需新增字段必须向后兼容，不得破坏性迁移
- 最近一次完成：无必需食材资料的菜品不再误标“库存可做”，添加菜品会显示“食材资料待完善”；AI 补充只加入库存可做候选，隔离副本实际点击午餐补齐 4 个营养槽位
- 当前风险：APP_ENV=production；PUSH_ENABLED=false；PushPlus token/topic 为空；Mac 本地 8080/8090 不恢复；本地分支仍领先 GitHub origin/main
- 下一步：继续保持不自动同步 GitHub；后续补齐菜品管理器中仍缺少必需食材资料的菜品
